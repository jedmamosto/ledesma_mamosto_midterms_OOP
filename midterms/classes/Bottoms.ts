...
export class Bottoms extends Clothing {
  private _isPants: boolean;

  private _isJeans: boolean;
  ...
  constructor(
    ...
    this._isPants = isPants;
    this._isJeans = isJeans;
  }
  ...
  get isPants(): boolean {
    return this._isPants;
  }

  set isPants(changePants: boolean) {
    this._isPants = changePants;
  }

  get isJeans(): boolean {
    return this._isJeans;
  }

  set isJeans(changeJeans: boolean) {
    this._isJeans = changeJeans;
  }
  ...
  repair(): void {
    ...
    if (this.isPants) {
      regenAmount = Math.ceil(this.maxHp * 0.18);
    }
  ...
  wash(): void {
    ...
    if (this.isPants) {
      regenAmount = Math.ceil(this.maxMana * 0.15);
    }
  ...
  // unique method to class Bottoms
  specialSkill(opponent: Clothing): void {
    console.log('Special skill: Jeans Supremacy!');
    if (this.isJeans) {
      const steal = this.price * 0.5;
      if (this.mana < 100) {
        ...
        opponent.hp -= steal;
        this.hp += steal;
        console.log(`${this.productName} stole ${steal} hp from ${opponent.productName}!`);
        console.log(`${opponent.productName} has ${opponent.hp} mana remaining.`);
      }
    } else {
      console.log('Sorry, Jeans Supremacy is only for jeans.');
    }
  }
}
